...
def countdown(count):
    ...
    format_sec = str()
    if count_sec < 10:
        format_sec = f"0{count_sec}"
    else:
        format_sec = count_sec
    canvas.itemconfig(timer_text, text=f"{count_min}:{format_sec}")
    if count > 0:
...
# Buttons
start = Button(text="Start", bg=GREEN, font=FONT_NAME, activebackground=GREEN, highlightthickness=0, command=start_stimer)
